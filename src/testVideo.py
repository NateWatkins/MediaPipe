...
#Use python to extract the subject four letter numbers t
import os
import glob
import cv2
import re
#FpS rate for output video should be 15 fps created using the 
def make_video_from_frames(frame_dir, out_path, fps=15):
    # Get all image files
    #Take frames in order they a
    ...
    head_n = min(20, len(frames))
    print(f"Found {len(frames)} frames. First {head_n}: " + ", ".join(os.path.basename(p) for p in frames[:head_n]))
    if not frames:
        return
...
#This should be the folder where the frames are located
frame_folder = "/Users/sehyr/Desktop/KinectBackup/subject_1490D (wrist)/frames/rgb"
output_video = "/Users/sehyr/Desktop/MediaPipe/1490D_AO_Picnaming.mp4"

make_video_from_frames(frame_folder, output_video, fps=15)
